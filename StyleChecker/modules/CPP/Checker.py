...
def CheckFileName(currentFilePath:str):
    ...
    if fileName.count(' ') > 0:
        PrintFileError(currentFilePath, 'Incorrect file name.', 'File name cannot contain whitespace.', 'File name: ' + fileName)
        failed = True
...
def CheckForOperator(line:str, index:int, currentFilePath:str):
    ...
    singleRegex = '(\+|-|=|\/|%|==|!=|>=|<=|&&|\|\||\||\^|\+=|-=|\*=|\/=|%=|<<=|>>=|&=|\^=|\|=|<<|>>)'
    exceptionRegex = '(->|\+\+|--)|((-)[0-9]+)'
    exceptionRegex += '|("*' + singleRegex + '*")'
    exceptionRegex += '|(' + 'operator' + ')'
...
def CheckForForbiddenToken(line:str, index:int, currentFilePath:str):
    ...
    # Regex to check for using namespace std.
    if re.search('\\busing\\b\s*\\bnamespace\\b\s*\sstd\\b[^;]*\;$', line) is not None:
        PrintStyleError(index, line, currentFilePath, 'Forbidden token.', '\'using namespace std\' is not allowed.')
        failed = True
